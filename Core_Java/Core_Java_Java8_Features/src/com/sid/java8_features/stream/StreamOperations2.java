/**
 * 	Problem statment - Write a program to perform various stream operations 2
 */
...
public class StreamOperations2 
{
	...
	private static void test8(List<String> stringCollection) 
	{
		...
		boolean isPresent=streamSupplier.get().anyMatch(s -> true);
		if(isPresent)
			System.out.println("match found");
		else
			System.out.println("match not found");
		boolean isNoneMatchPresent= streamSupplier.get().noneMatch(s -> true);
		if(isNoneMatchPresent)
			System.out.println("none match found");
		else
			System.out.println("none match not found");
	}
